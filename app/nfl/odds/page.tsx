...
function NflOdds() {
  const { data: events, status } = useQuery({
    ...
    queryFn: () =>
      axios
        .get(
          `https://api.the-odds-api.com/v4/sports/americanfootball_nfl/odds?regions=us&markets=h2h,spreads,totals&oddsFormat=american&apiKey=${process.env.NEXT_PUBLIC_ODDS_API_KEY}`
        )
